...
均衡把衰落本身除掉了,但代价转嫁到噪声上:等效噪声功率变为 N₀/|h|²。瞬时 SNR = |h|²·平均SNR 服从指数分布,**任何平均 SNR 下深衰落(|h|→0)的概率都不为零**——这是 Rayleigh 性能差于 AWGN 的根本原因,也解释了为什么高 SNR 下差距依然存在(见下文观察)。
...
- v2 核心改造:整句单向量 → 逐 token 传输(解除速率瓶颈,这一步比换模型更关键)
