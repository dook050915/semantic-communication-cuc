...
数据来自 Europarl English,采样并过滤得到 50,000 条英文句子(保留 token 数 4–30 的句子,去除 Europarl 标签行、空行与括号形式的舞台说明)。与 stateSeq2Seq / Rayleigh 各实验同源、同切分(seed=42),保证跨实验可比。
...
- 逐 token 传输突破了整句单向量的容量瓶颈:速率对齐(token c16 vs 整句 c256,带宽相当)下,中高 SNR 段 token 决定性胜出(20 dB 3.2 倍),证明 stateSeq2Seq 绝对 BLEU 偏低的主因是传输粒度而非带宽。
- 代价出现在极低 SNR:固定带宽下整句单向量冗余更高、更抗噪,c16 因每 token 仅 8 个复符号而被噪声打穿,交叉点约在 0 dB。
- channel_dim 仍是信道容量,折中点落在 256 附近(低 SNR 明显好于 c16,高 SNR c128≈c256);attention_hdim 不敏感(内容不流经打分支路)。
- 功率归一化 PAD 污染修复后,曲线变化集中在低 SNR、高 SNR 几乎不变,定量验证了对该 bug 影响的事前预测。
